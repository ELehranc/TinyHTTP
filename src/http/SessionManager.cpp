...
std::shared_ptr<Session> SessionManager::getSession(const HttpRequest &req, HttpResponse *resp)
{
    ...
    {
        LOG_INFO("新的会话到达,生成SessionID,塞入cookie");
    ...
    session->refresh();
    storage_->save(session); // 这里是不是有问题，需要确保正确保存会话？
    return session;
}
...
void SessionManager::cleanExpiredSessions()
{
    ...
    // try
    // {
    //     std::vector<std::string> allSessionIds = storage_->getAllSessionIds();

    //     for (const auto &sessionId : allSessionIds)
    //     {
    ...
    // {
    //     LOG_ERROR("Error cleaning up sessions: {}", e.what());
    // }
}
...
std::string SessionManager::getSessionIdFromCookie(const HttpRequest &req)
{
    ...
    if (!cookie.empty())
    {
        ...
        if (pos != std::string::npos)
        {
            ...
            pos += 10;                          // 跳过sessionId=
            size_t end = cookie.find(';', pos); // Cookie: key1=value1; key2=value2; key3=value3\r\n
